...
def create_savings_goal(goal: SavingsGoal):
    ...
    try:
        ...
        if cursor.fetchone()[0] > 0:
            raise HTTPException(status_code=400, detail="A savings goal already exists")
        
        cursor.execute('''
            INSERT INTO savings_goals (target_amount, current_amount, monthly_savings, target_date, description)
            VALUES (%s, %s, %s, %s, %s)
            RETURNING target_amount, current_amount, monthly_savings, target_date, description
        ''', (
            goal.target_amount,
            goal.current_amount,
        ...
        return {
            "target_amount": new_goal[0],
            "current_amount": new_goal[1],
            "monthly_savings": new_goal[2],
            "target_date": new_goal[3],
            "description": new_goal[4]
        }
    except Exception as e:
...
def get_savings_goal():
    ...
    try:
        ...
        cursor.execute('SELECT target_amount, current_amount, monthly_savings, target_date, description FROM savings_goals LIMIT 1')
        ...
        return {
            ...
            "current_amount": goal[1],
            "monthly_savings": goal[2],
            "target_date": goal[3],
            "description": goal[4]
        }
    except Exception as e:
...
def update_savings_goal(goal: SavingsGoal):
    ...
    try:
        ...
        cursor.execute('''
            ...
            SET target_amount = %s, monthly_savings = %s, target_date = %s, description = %s
            RETURNING target_amount, current_amount, monthly_savings, target_date, description
        ''', (
            goal.target_amount,
            goal.monthly_savings,
        ...
        if not updated_goal:
            raise HTTPException(status_code=404, detail="No savings goal found to update")
            
        conn.commit()
        return {
            "target_amount": updated_goal[0],
            "current_amount": updated_goal[1],
            "monthly_savings": updated_goal[2],
            "target_date": updated_goal[3],
            "description": updated_goal[4]
        }
    except Exception as e:
...
def create_savings_transaction(transaction: SavingsTransaction):
    conn = None
    try:
        conn = get_db()
        cursor = conn.cursor()
        ...
        updated_amount = cursor.fetchone()
        ...
        cursor.execute('''
            ...
            VALUES (%s, %s, %s, %s)
            RETURNING amount, type, date, description
        ''', (
            transaction.amount,
            transaction.type,
            transaction.date,
        ...
        return {
            "amount": new_transaction[0],
            "type": new_transaction[1],
            "date": new_transaction[2],
            "description": new_transaction[3]
        }
    except Exception as e:
...
def create_abstinence_tracker(tracker: AbstinenceTracker):
    ...
    try:
        ...
        cursor.execute('SELECT COUNT(*) FROM abstinence_trackers')
        if cursor.fetchone()[0] > 0:
            raise HTTPException(status_code=400, detail="An abstinence tracker already exists")
        
        cursor.execute('''
            INSERT INTO abstinence_trackers (start_date, end_date, current_streak, longest_streak, total_days)
            VALUES (%s, %s, %s, %s, %s)
            RETURNING start_date, end_date, current_streak, longest_streak, total_days
        ''', (
            tracker.start_date,
            tracker.end_date,
        ...
        new_tracker = cursor.fetchone()
        conn.commit()
        
        return {
            "start_date": new_tracker[0],
            "end_date": new_tracker[1],
            "current_streak": new_tracker[2],
            "longest_streak": new_tracker[3],
            "total_days": new_tracker[4]
        }
    except Exception as e:
...
def get_abstinence_tracker():
    ...
    try:
        ...
        cursor.execute('SELECT start_date, end_date, current_streak, longest_streak, total_days FROM abstinence_trackers LIMIT 1')
        ...
        raise HTTPException(status_code=500, detail="Failed to fetch abstinence tracker")
    finally:
        if conn:
            conn.close()
...
def update_abstinence_tracker(tracker: AbstinenceTracker):
    ...
    try:
        ...
        cursor.execute('''
            ...
            SET start_date = %s, end_date = %s, current_streak = %s, longest_streak = %s, total_days = %s
            RETURNING start_date, end_date, current_streak, longest_streak, total_days
        ''', (
            tracker.start_date,
            tracker.end_date,
        ...
        updated_tracker = cursor.fetchone()
        if not updated_tracker:
            raise HTTPException(status_code=404, detail="No abstinence tracker found to update")
            
        conn.commit()
        return {
            "start_date": updated_tracker[0],
            "end_date": updated_tracker[1],
            "current_streak": updated_tracker[2],
            "longest_streak": updated_tracker[3],
            "total_days": updated_tracker[4]
        }
    except Exception as e:
        logger.error(f"Error updating abstinence tracker: {e}")
        if conn:
            conn.rollback()
        raise HTTPException(status_code=500, detail=str(e))
    finally:
...
# Abstinence Checkins Endpoints
@app.post("/abstinence/checkin/", response_model=AbstinenceCheckin)
def create_abstinence_checkin(checkin: AbstinenceCheckin):
    conn = None
    try:
        conn = get_db()
        cursor = conn.cursor()
        ...
        # Update the tracker based on checkin success
        if checkin.success:
            ...
            ''', (new_streak, new_longest, new_total))
        else:
            cursor.execute('''
                UPDATE abstinence_trackers
        ...
        return {
            "date": new_checkin[0],
            "success": new_checkin[1],
            "notes": new_checkin[2]
        }
    except Exception as e:
        logger.error(f"Error creating abstinence checkin: {e}")
        if conn:
            conn.rollback()
        raise HTTPException(status_code=500, detail=str(e))
    finally:
...
def get_abstinence_checkins():
    ...
    try:
        ...
        logger.error(f"Error fetching abstinence checkins: {e}")
        raise HTTPException(status_code=500, detail="Failed to fetch abstinence checkins")
    finally:
        if conn:
            conn.close()

# Progress Endpoints
@app.get("/savings/progress/", response_model=ProgressStats)
def get_savings_progress():
    ...
    try:
        ...
        cursor.execute('SELECT target_amount, current_amount, monthly_savings, target_date FROM savings_goals LIMIT 1')
        goal = cursor.fetchone()
        
        if not goal:
            raise HTTPException(status_code=404, detail="No savings goal found")
            
        target_amount, current_amount, monthly_savings, target_date = goal
        ...
        # Check if on track
        expected_amount = monthly_savings * (months_remaining)
        ...
        raise HTTPException(status_code=500, detail="Failed to calculate savings progress")
    finally:
        if conn:
            conn.close()
...
def get_abstinence_progress():
    ...
    try:
        ...
        # Get the tracker
        cursor.execute('SELECT start_date, end_date, total_days FROM abstinence_trackers LIMIT 1')
        tracker = cursor.fetchone()
        
        if not tracker:
            raise HTTPException(status_code=404, detail="No abstinence tracker found")
            
        start_date, end_date, total_days = tracker
        ...
        days_completed = min((today - start_date).days, total_days_planned)
        days_remaining = max(0, total_days_planned - days_completed)
        
        # Calculate success rate
        cursor.execute('SELECT COUNT(*) FROM abstinence_checkins WHERE success = true')
        successful_days = cursor.fetchone()[0]
        success_rate = (successful_days / days_completed * 100) if days_completed > 0 else 0
        ...
        raise HTTPException(status_code=500, detail="Failed to calculate abstinence progress")
    finally:
        if conn:
            conn.close()

# Calendar Endpoint
@app.get("/abstinence/calendar/")
def get_abstinence_calendar(year: int, month: int):
    conn = None
    try:
        conn = get_db()
        cursor = conn.cursor()
        
        # Get all checkins for the specified month
        ...
        raise HTTPException(status_code=500, detail="Failed to generate abstinence calendar")
    finally:
        if conn:
            conn.close()
